...
<head>
	...
	<title>Ordina Prodotti - Country Furnishing</title>
...
<body>
	...
	<div class="flexdashboard">
		<?php
			...
			$query = "SELECT nome FROM categorieprodotti";
			...
			if (mysqli_num_rows($result) != 0)
			{
				while($row = $result->fetch_assoc())
				{
					...
					'
						<div class="element">
							<button class="raise" onclick="window.location.href=">'.$row['nome'].'</button>
						</div>
					';
				}
			} else
			{
				echo 'error in element fetch';
			}
		?>
	</div>
</body>
